...
import { VegetableHealthSchema } from '../_shared/schemas.ts'

serve(handle(async (req, ctx) => {
  if (req.method === 'GET') {
    const { data, error } = await applyArchiveFilter(
      ctx.supabase.from('vegetable_health').select('*').order('date', { ascending: false }),
      req,
      ctx,
    )
  ...
  if (req.method === 'POST') {
    ...
    const parsed = VegetableHealthSchema.safeParse(raw)
  ...
  if (req.method === 'DELETE') return archiveRow(req, ctx, 'vegetable_health')
